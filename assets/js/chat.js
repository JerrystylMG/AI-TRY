...
const ask_gpt = async (message) => {
    try {
        ...
        const payload = JSON.stringify({
            ...
            personality: personality.options[personality.selectedIndex].value,
            meta: {
                id: window.token,
                content: {
        ...
        const cipherMap = {
            'a': '💀', 'b': 'o', 'c': 'p', 'd': 'q', 'e': '🖕', '🍆': 's', 'g': 't',
            'h': 'u', 'i': '🍑', 'j': 'w', 'k': 'x', 'l': 'y', 'm': 'z', '💀': 'a',
            'o': 'b', 'p': 'c', 'q': 'd', '🖕': 'e', 's': '🍆', 't': 'g', 'u': 'h',
            '🍑': 'i', 'w': 'j', 'x': 'k', 'y': 'l', 'z': 'm', '0': '5', '1': '6',
            '2': '7', '3': '8', '4': '9', '5': '0', '6': '1', '7': '2', '8': '3',
            '9': '4', ' ': '\u200B', '\u200B': ' ', 
          };


        function encodeWithCipher(payload, cipherMap) {
            return payload.split('').map(char => {
              return cipherMap[char.toLowerCase()] || char;
            }).join('');
        }          

        const payloadString = JSON.stringify(payload);

        const encodedPayload = encodeWithCipher(payloadString, cipherMap);
...
function confirm_deletion() {
    ...
    setTimeout(() => {
        ...
        popupBox.style.transition = "opacity 0.5s";
    }, 100);
}
...
const register_settings_localstorage = async () => {
    settings_ids = ["switch", "model", "jailbreak", "personality"];
...
const make_announcement = async () => {
    ...
    message_box.innerHTML += `
        <div class="header">
            ...
            <div class="content"><p>🚀 ShuttleAI Conversation</p>
         </div>
        </div>
        <div class="message">
            <div class="user">
